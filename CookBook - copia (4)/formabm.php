...
function formularioEtiqueta($abm="Etiqueta"){
	if (isset($_GET["id"])){
		$idEtiqueta=$_GET["id"];
...
function formularioAutor($abm="Autor"){
	if (isset($_GET["id"])){
					$idAutor=$_GET["id"];
...
function formularioLibro($abm="Libro"){
	if (isset($_GET["id"])){
		$idlibro=$_GET["id"];
...
function contenido(){
	if(isset($_GET["abm"])){
		...
		echo '<input type="hidden" name="abm" value="'.$abm.'" />'; //oculto para alta y modificacion
		...
		echo '<br>';
		echo '<span style="float:right" id= "obligatorio">Los campos con * deben llenarse obligatoriamente</span>';
		echo '</fieldset>';
		echo '</form>';	
	}
}
